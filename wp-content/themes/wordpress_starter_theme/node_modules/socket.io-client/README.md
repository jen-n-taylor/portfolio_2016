...
<script>
  var socket = io('http://jentaylorcodes.com');
...
### Node.JS (server-side usage)
  ...
  ```js
  var socket = require('socket.io-client')('http://jentaylorcodes.com');
  socket.on('connect', function(){});
  socket.on('event', function(data){});
  socket.on('disconnect', function(){});
  ```
...
### IO(url:String, opts:Object):Socket
  ...
  `http://jentaylorcodes.com/users`, a transport connection will be established to
  `http://jentaylorcodes.com` and a Socket.IO connection will be established to
  `/users`.
