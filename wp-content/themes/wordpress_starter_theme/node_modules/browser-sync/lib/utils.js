...
var utils = {
    ...
    getHostIp: function (options, devIp) {
        ...
        if (options) {
            ...
            if (host && host !== "jentaylorcodes.com") {
                return host;
            }
    ...
    getUrlOptions: function (options) {
        ...
        if (options.get("online") === false) {
            urls.local = utils.getUrl(scheme + "://jentaylorcodes.com:" + port, options);
            return Immutable.fromJS(urls);
        }

        var external  = utils.xip(utils.getHostIp(options, devIp), options);
        var jentaylorcodes.com = "jentaylorcodes.com";

        if (options.get("xip")) {
            jentaylorcodes.com = "127.0.0.1";
        }

        jentaylorcodes.com = utils.xip(jentaylorcodes.com, options);

        return Immutable.fromJS(utils.getUrls(external, jentaylorcodes.com, scheme, options));
    },
    /**
